...
protected:
	...
	size_t write(const char& c) {
		...
		while (!usart::buffer.tx.enqueue(c))
			flush();
	...
	void flush() {
		ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
			while (bit_is_set(USART_CSRB, USART_BIT_DRIE) ||
				   bit_is_clear(USART_CSRA, USART_BIT_TXC)) {
				if (bit_is_set(USART_CSRB, USART_BIT_DRIE) &&
					bit_is_clear(SREG, SREG_I)) {
